...
local lib = Library:new{ name='plugin.gamedonia', publisherId='com.gamedonia' }
...
Request.put = function(url, content, auth, sessionToken, gameid, callback)
    local urlBuf = string.format("%s/%s%s", lib.getApiServerUrl(), lib.getApiVersion(), url)
...
-- print ("GamedoniaStore - store: "..storeIn)
...
Store.init = function()
    ...
    if not store.canMakePurchases or not store.canLoadProducts then
        store.init(storeIn, callbacks.listener)
    else
...
Store.requestProducts = function(listOfProducts, callback)
    -- print ("GamedoniaStore.requestProducs")
    callbacks.userRequestCB = callback
    Store.init()
    if storeIn == "google" then
        -- Fer la compra directe.
        callbacks.userRequestCB(listOfProducts)
    elseif store.canLoadProducts then
        --[[ print("list of products")
        printTable(listOfProducts) --]]
        store.loadProducts(listOfProducts, callbacks.requestProducts)
    else
        print ("GamedoniaStore.requestProducs - Error")
    end
end
...
-- Buy product
------------------
Store.buyProducts = function(listOfProducts, callback)
