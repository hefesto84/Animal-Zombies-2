...
local sceneInstance	-- reference to currently loaded scene
local overlayInstance	-- reference to currently loaded overlay 

local isInScene	-- is in scene or there's any scene transition?
...
storyboard.removeOnSceneChange = false -- if true, will automatically remove non-active scenes on scene change
...
function storyboard.isInScene()
	return isInScene
end

-----------------------------------------------------------------------------------------

function storyboard.getCurrentSceneOrOverlay()
...
local function nextTransition( sceneGroup, fx, effectTime, touchOverlay, oldScreenshot, customParams )
	...
	local disableOverlay = function()
		...
		if currentModule and storyboard.scenes[currentModule] then
			...
			if storyboard.removeOnSceneChange then
				storyboard.removeAll()
			elseif storyboard.purgeOnSceneChange then
				storyboard.purgeAll()
			end
		end
	end
...
local function purgeLruScene( event )	-- Lru = "least recently used"
	...
	elseif storyboard.isDebug and not storyboard.removeOnSceneChange and not storyboard.purgeOnSceneChange then
