...
local function onTouch(event)
    ...
    if event.phase == "release" or event.phase == "ended" then
		...
		if event.isBackKey or (id == "btnClose" and event.target.isWithinBounds) then
            ...
            elseif scene.params.isRefillWeapon then
                AZ.S.showOverlay("popups.popuprefillweapon", options)
            elseif scene.params.isSlotPopup then
                options.params.isSlotPopup = nil
                AZ.S.showOverlay("popups.popupslottwo", options)
            end
        elseif id == "btnBuy" then
			...
			local function callback(success, receipt)
				if success then
					...
					print("Coins earned: ".. scene.quantity)
			...
			AZ.Gamedonia:buyProduct(scene.storeID, callback, true)
...
local function setItemToBuy()
    local price = nil
    if scene.params.isLollipop then
        price = 10
    elseif scene.params.isRefillWeapon or scene.params.isPopupSlot then
            ...
            if AZ.shopInfo.shop.weapons[i].name == scene.params.weaponName.."_name" then
				for j = 1, #AZ.userInfo.weapons do
					if AZ.userInfo.weapons[j].name == scene.params.weaponName then
						price = AZ.shopInfo.shop.weapons[i].boosterData[AZ.userInfo.weapons[j].boughtLevel].gameplayPrice
					end
				end
            end
        end
    ...
    for i = 1, #AZ.bankInfo.bankShop.bank-2 do
        if (tonumber(AZ.bankInfo.bankShop.bank[i].quantity + AZ.userInfo.money)) > price then
            scene.itemToBuy = AZ.bankInfo.bankShop.bank[i].itemName
            scene.quantity = tonumber(AZ.bankInfo.bankShop.bank[i].quantity)
            scene.price = AZ.bankInfo.bankShop.bank[i].price
			scene.storeID = AZ.bankInfo.bankShop.bank[i].storeID
            return
        end
    end
end
...
function scene:init()
	...
	scene.Popup.background:addEventListener("touch", scene.noTouch)
    ...
    scene.Popup.txtNoCoins = display.newText({text = AZ.utils.translate("pp_need_coins"), x = _W*0.5, y = _H*0.5-140*_R, font = BRUSH_SCRIPT, fontSize = 45*_R, align = "center"})
    ...
    scene.Popup.itemGroup.img = display.newImage(scene.myImageSheet, sheetInfo:getFrameIndex(scene.itemToBuy))
    scene.Popup.itemGroup.img:scale(_R, _R)
    scene.Popup.itemGroup.img.x, scene.Popup.itemGroup.img.y = _W*0.5, _H*0.5-25*_R
    scene.Popup.itemGroup:insert(scene.Popup.itemGroup.img)
    scene.Popup.itemGroup.txt = display.newText({text = "+"..tostring(scene.quantity), x = _W*0.5, y = _H*0.5+90*_R, font = INTERSTATE_BOLD, fontSize = 40*_R, align = "center"})
    ...
    scene.Popup.btnBuy = AZ.ui.newEnhancedButton2{
        ...
        text1 = {text = tostring(scene.price), fontName = INTERSTATE_BOLD, fontSize = 50, X = 0, Y = 30, color = FONT_BLACK_COLOR},
        onEvent = onTouch,
        id = "btnBuy"
    }
